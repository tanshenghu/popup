define("widget/popup/1.0.0/popup-debug", [ "$-debug" ], function(require, exports, module) {
    var $ = require("$-debug"), doc = document, body = "body", // 弹出 浮层
    popup = function(param) {
        var hand = param.hand instanceof Array ? param.hand[1] : $(param.hand), // 考虑到事件委托，所以param可传['body', '.okbtn']
        box = $(param.box), cover = typeof param.cover == "undefined" ? true : param.cover, zIndex = param.zIndex || 1e3, drag = param.drag, fixed = param.fixed === true ? "fixed" : "absolute", CW = 0, CH = 0, boxW = parseInt(param.width), boxH = parseInt(param.height), sclT = 0, sclL = 0, callback = param.callback, hidefn = param.hidefn, okfn = param.okfn, trigger = null, // 当前浮层对应的hand
        Body = $(body);
        if (!box.length) {
        ...
        var posFn = function(resetPos) {
            ...
            var left = (CW - boxW) * .5 < 0 ? 0 : (CW - boxW) * .5 > CW - boxW ? CW - boxW : (CW - boxW) * .5;
            var top = (CH - boxH) * .5 < 0 ? 0 : (CH - boxH) * .5 > CH - boxH ? CH - boxH : (CH - boxH) * .5;
            var css = {
                position: fixed,
                "z-index": zIndex,
        ...
        var _show = function(targ) {
            ...
            var $this = $(_curHideBtn), total = Body.children('[poptype="tsh-popbox"]'), surplus = total.not(box), ispop = total.length - 1 === total.filter(":hidden").length || surplus.filter('[cover="true"]:visible').length === 0;
            // 判断 关闭 事件来源于按钮
            ...
            box.fadeOut(300);
            typeof hidefn === "function" && hidefn.apply(trigger, [ this, box, !!(_curHideBtn && _curHideBtn.nodeName) ]);
            // 第三个参数主要是判断是否手动调用hide方法
            return box;
        };
        // 委托方式 去操作浮层
        ...
        // 拖动
        function dragger() {
            var dragBar = box.find(".dialog-title"), _O = {
                ON: false
            };
            $(doc).on("mouseup", function() {
                ...
                ev = ev || event;
                if (_O.ON) {
                    var l = Math.min(ev.clientX - _O.L, _O.BW), t = Math.min(ev.clientY - _O.T, _O.BH);
                    l = Math.max(l, 0);
                    t = Math.max(t, 0);
                    _O.dialog.css({
                        left: l + "px",
